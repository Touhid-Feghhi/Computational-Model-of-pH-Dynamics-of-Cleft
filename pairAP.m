...
elseif nsim==2
    ...
    releaseyes=1;
    tstart=400;
    tend=tfn/stim_freq-400;
elseif nsim==3
    conti =1; 
    releaseyes=1;
    tstart=tfn/stim_freq;
    tend=tfn -( tfn/stim_freq-400);
end
...
peak1p =0.4*10^(-6);%*(heaviside(t - td)); % [M] Ca microdomain initila peak
peak2 = 2.5*10^(-6);%66*peak1;%*(heaviside(t - td)); % [M] Ca microdomain initila peak
peak2p = 2.5*10^(-6);%66*peak1;%*(heaviside(t - td)); % [M] Ca microdomain initila peak
k1=1.5*10^(2); % 1/M.us
k2= 15*10^(-6); % 1/us
%k3 = 12*10^(-6); %1/us
k3 = 100*10^(-6); %1/us
...
pretwo=peak1p*((1-s1p)*(1 - exp(-(t-td1p)/trise1))*(heaviside(t-td1p))+s1p*exp(-(t-tp1p)/tau1))*(heaviside(t-td1p));%[Ca2+] vs time
postone=peak2*(((1-s2)*(1 - exp(-(tnew-td2)/trise2))*(heaviside(tnew-td2))+s2*exp(-(tnew-tp2)/tau2)))*(heaviside(tnew-td2));%[Ca2+] vs time
posttwo=peak2p*(((1-s2p)*(1 - exp(-(t-td2p)/trise2))*(heaviside(t-td2p))+s2p*exp(-(t-tp2p)/tau2)))*(heaviside(t-td2p));%[Ca2+] vs time
if nsim==1
    pre=preone;%pretwo ;%+ pretwo ;
    post=0;
elseif nsim==2
    pre=preone ;
    post=postone;
