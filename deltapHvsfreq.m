...
releaseyes=1;
conti =0;
%stimfreq=2;%stimulation frequency [Hz]
%stimperiod = (0.5*tfn)/stimfreq;
stimperiod = ((tfn)/freq);
tstart=((stimnum)-1)*stimperiod;
maxstim=2;
if stimnum== maxstim
   tend=tfn;
else
...
%%%%%% phosphate %%%%%%
%k45= 1.4111*10^(8);
k45=1.2589*10^(8); %forward rate of phosphate buffer consuming protons   [1/mM*us]
...
k1011=k1110*2.9512*10^(3);% rate of consumption of ATP in reaction ATP + H >< HATP
k1312=10^(4); % rate of consumption of H2ATP in reaction HATP + H >< H2ATP
k1213=k1312*12.589; %rate of consumption of HATP in reaction HATP + H >< H2ATP
%%%%%%%%%%%%%%%%%%%%%% Initial Concentrations %%%%%%%%%%%%%%%%%%%%%
initialph=7.2;%7.239 %7.28; % The initial pH which the cleft is just before the release of protons
svradius= 0.01342; %[um]
cleftwidth=0.013; %[um]
...
%%%%%% Released ATP %%%%%%
TotalATP=0;%104;%150;%2384;
releasedH2ATP =0.0347*TotalATP; %%%%concentration of H2ATP
releasedHATP =0.8719*TotalATP; %%%%concentration of HATP
releasedATP =0.0934*TotalATP; %%%%concentration of ATP
...
phosphateioninitial=0;%0.5; %initial value of phosphate  ion  buffer  [A]_0 [mM]
...
Datamiddle=[tnew,PH(:,SR-28),c(:,SR-28),c(:,2*SR-28),c(:,3*SR-28),c(:,4*SR-28),c(:,5*SR-28),c(:,6*SR-28),c(:,7*SR-28),c(:,8*SR-28),c(:,9*SR-28),c(:,10*SR-28),c(:,11*SR-28),c(:,12*SR-28)];
xlswrite('datamid.xls', row_header,'Sheet1','A1')
xlswrite('datamid.xls', Datamiddle,'Sheet1','A2')
Datasource=[tnew,PH(:,2),c(:,2),c(:,1*SR+2),c(:,2*SR+2),c(:,3*SR+2),c(:,4*SR+2),c(:,5*SR+2),c(:,6*SR+2),c(:,7*SR+2),c(:,8*SR+2),c(:,9*SR+2),c(:,10*SR+2),c(:,11*SR+2)];
...
s(:,:)=zeros(12*SR,1);
ts=0;
s(1,1)=heaviside(t-ts)*1*releasedproton*(1/tau_sv)*exp(-(t-ts)/tau_sv) ;
s(2,1)=heaviside(t-ts)*0.5*releasedproton*(1/tau_sv)*exp(-(t-ts)/tau_sv);
%%%% Glutamate Release %%%%%
...
peak2 = 1.5*10^(-6);%66*peak1;%*(heaviside(t - td)); % [M] Ca microdomain initila peak
k1=1.5*10^(2); % 1/M.us
k2= 15*10^(-6); % 1/us
%k3 = 12*10^(-6); %1/us
k3 = 100*10^(-6); %1/us
...
preone=peak1*((1-s1)*(1 - exp(-(tnew-td1)/trise1))*(heaviside(tnew-td1))+s1*exp(-(tnew-tp1)/tau1));%[Ca2+] vs time
pretwo=peak1*((1-s1p)*(1 - exp(-(t-td1p)/trise1))*(heaviside(t-td1p))+s1p*exp(-(t-tp1p)/tau1));%[Ca2+] vs time
postone=peak2*(0.988*((1-s2)*(1 - exp(-(tnew-td2)/trise2))*(heaviside(tnew-td2))+s2*exp(-(tnew-tp2)/tau2)));%[Ca2+] vs time
posttwo=peak2*(0.988*((1-s2p)*(1 - exp(-(t-td2p)/trise2))*(heaviside(t-td2p))+s2p*exp(-(t-tp2p)/tau2)));%[Ca2+] vs time
if stimnum==1
    pre=preone;
    post=postone;
else
