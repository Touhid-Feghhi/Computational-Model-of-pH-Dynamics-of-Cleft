...
freq=21;
stimperiod = ((tfn)/freq)
if APnumber ==1 && nsim==1
    tstart =0;
    tend=400+(AZ-1)*stimperiod;
    releaseyes=0;
elseif APnumber ==1 && nsim==2
    tstart=  400+(AZ-1)*stimperiod;
    tend=9*stimperiod ;
    releaseyes=1;
elseif APnumber==2 
    tstart= 9*stimperiod +stimperiod*(AZ-1)+400;
    if AZ==1 | AZ==2
        tend= 9*stimperiod ;
        releaseyes=1;
    else 
...
elseif APnumber==3
    if AZ==1
    tstart= (18)*stimperiod +400 ;
    tend=tfn;
    releaseyes=1;
    elseif AZ==2        
    tstart= (19)*stimperiod +400 ;
    tend=tfn;
    releaseyes=1;
    end
end
...
%%%%%% phosphate %%%%%%
k45=3.1623*10^(8); %forward rate of phosphate buffer consuming protons   [1/mM*us]
...
Datamiddle=[tnew,PH(:,SR-27),c(:,SR-27),c(:,2*SR-27),c(:,3*SR-27),c(:,4*SR-27),c(:,5*SR-27),c(:,6*SR-27),c(:,7*SR-27),c(:,8*SR-27),c(:,9*SR-27),c(:,10*SR-27),c(:,11*SR-27),c(:,12*SR-27)];
...
elseif APnumber==2 
   xlswrite('mid2.xls', row_header,'Sheet1','A1')
   xlswrite('mid2.xls', Datamiddle,'Sheet1','A2')
 %  xlswrite('source2.xls', row_header,'Sheet2','A1')
 %  xlswrite('source2.xls', Datasource,'Sheet2','A2')
elseif APnumber==3 
   xlswrite('mid3.xls', row_header,'Sheet1','A1')
   xlswrite('mid3.xls', Datamiddle,'Sheet1','A2')
%   xlswrite('source3.xls', row_header,'Sheet2','A1')
%   xlswrite('source3.xls', Datasource,'Sheet2','A2') 
...
tau2=50*10^3; %us
peak1=3*0.4*10^(-6);
peak11 =0.4*10^(-6);%*(heaviside(t - td)); % [M] Ca microdomain initila peak
peak12 =3*0.4*10^(-6);%*(heaviside(t - td)); % [M] Ca microdomain initila peak
...
s11=0.5+0.5*tanh((tnew-tp11)/b11);%smoothing function
s12=0.5+0.5*tanh((tnew-tp12)/b12);%smoothing function
...
%preone=(peak11)*(1 - exp(-(tnew-td11)/trise11))*(1-s11)*heaviside(tnew-td11)+ s11*peak11*exp(-(tnew-tp11)/tau11)+(peak12)*(1 - exp(-(tnew-td12)/trise12))*(1-s12)*heaviside(tnew-td12)+ s12*peak12*exp(-(tnew-tp12)/tau12);

preone=peak1*((1-s1)*(1 - exp(-(tnew-td1)/trise1))*(heaviside(tnew-td1))+s1*exp(-(tnew-tp1p)/tau1))*(heaviside(tnew-td1));%[Ca2+] vs time
postone=peak2*(((1-s2)*(1 - exp(-(t-td2)/trise2))*(heaviside(t-td2))+s2*exp(-(t-tp2)/tau2)))*(heaviside(t-td2));%[Ca2+] vs time
